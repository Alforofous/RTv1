...
void	draw_image8(void *param)
{
	...
	if (utils->light_render == 1)
		draw_rectf(&(t_pxl_func){&put_pixel, utils->curr_img},
			...
			utils->curr_img->dim.size.y - 2}, 0xBF5A00);
	draw_rect(&(t_pxl_func){&put_dot, utils},
		(t_2i){0, 0}, (t_2i){utils->curr_img->dim.size.x - 2,
		utils->curr_img->dim.size.y - 2}, 0xFFC000);
	coords[0].x = (int)((float)utils->curr_img->dim.size.x * 0.5f);
	coords[0].y = (int)((float)utils->curr_img->dim.size.y * 0.45f);
	draw_circlef(&(t_pxl_func){&put_pixel, utils->curr_img},
		&(t_2i){coords[0].x, coords[0].y}, (coords[0].x + coords[0].y) / 4, 0xFFC000);
	coords[0].x = (int)((float)utils->curr_img->dim.size.x * 0.5f);
	coords[0].y = (int)((float)utils->curr_img->dim.size.y * 0.5f);
	coords[1].x = (int)((float)utils->curr_img->dim.size.x * 0.5f);
	coords[1].y = (int)((float)utils->curr_img->dim.size.y * 0.8f);
	if (utils->light_render == 1)
